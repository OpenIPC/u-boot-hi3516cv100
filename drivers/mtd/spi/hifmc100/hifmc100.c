					...
					unsigned char level)
{
	...
	FMC_PR(BP_DBG, "* Start BP %s level %d\n", (cmp ? "bottom" : "top"),
			level);
...
void hifmc100_spi_lock(unsigned char cmp, unsigned char level)
{
	...
	if ((cmp == BP_CMP_BOTTOM) && (host->cmp == BP_CMP_TOP)) {
		DB_MSG("Warning: Set bottom option will change a OTP(One ");
		puts("Time Program) bit!\nThere is no way to recover if ");
		puts("them are changed.\nAre sure of what you are doing!\n");
	...
	if ((level != BP_LEVEL_0) && (level != BP_LEVEL_MAX)
		...
		puts("Program) bit when it is changed!\n");
		return;
	}
